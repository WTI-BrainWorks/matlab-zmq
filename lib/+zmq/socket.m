function varargout = socket(varargin)
% zmq.ctx_new - Create a ZMQ socket
%
% Usage: socket = zmq.socket(context, type)
%
...
%                             use the ZMQ_SUBSCRIBE option of zmq.setsockopt to specify which
...
%         endpoint (see zmq.connect), or at least one endpoint must be created for accepting
%         incoming connections with (see zmq.bind).

    [varargout{1:nargout}] = zmq.zmqcore('socket', varargin{:});
end
